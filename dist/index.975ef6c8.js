...
function handlerInput(evt) {
    ...
    (0, _fetchCountries.fetchCountries)(value).then((data)=>{
        if (data.length > 10) {
            clearHTML();
...
function createMarkup(countriesArray) {
    ...
    if (countriesArray.length === 1) {
        const { name: { official  } , capital , population , flags: { svg  } , languages ,  } = countriesArray[0];
...
function fetchCountries(name) {
    return fetch(`${BASE_URL}${ENDPOINT}/${name}?fields=name,capital,population,flags,languages`).then((response)=>{
